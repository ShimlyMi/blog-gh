...
const { publishTalk, updateTalk, deleteTalkById, toggleTalkPublic,
...
class TalkController {
    ...
    async togglePublic(ctx) {
        ...
        try {
            let res = await toggleTalkPublic(id, status)
            ctx.body = result(message + "说说成功", res)
        } catch (err) {
            console.error(err)
            return ctx.app.emit("error", throwError(errorCode, message + "说说失败"), ctx)
    ...
    async getTalkList(ctx) {
        try {
            let res = await getTalkList(ctx.request.body)
