const { createArticleTags } = require("../service/articleTag.service");
const { getOneCategory, createCategory } = require("../service/category.service");
const { createTag, getOneTag } = require("../service/tag.service");
/**
 * 新增和编辑文章关于分类的公共方法
 * @param {*} id
 * @param {*} category_name
...
const createCategoryOrReturn = async (id, category_name) => {
    ...
    if (id) {
        ...
        if (oneTag) {
            ...
            let newTag = createCategory({ category_name });
            finalId = newTag.id;
        }
    }
